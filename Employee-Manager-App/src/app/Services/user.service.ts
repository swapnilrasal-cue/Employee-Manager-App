...
export class UserService {
  ...
  onCreateUser(newUser : User){
    return this.http.post<LoginResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signUp?key='+this.apiKey,
      {
        email : newUser.email,
      ...
      .pipe(tap(responseData =>{
        ...
        return this.http.post(this.apiUrl + 'users'+responseData.localId+'.json', newUser);
        // console.log(newUser);
  ...
  onLogin(email : string, password : string ){
    ...
    return this.http.post<LoginResponseData>('https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key='+this.apiKey,
      {
        email : email,
        password : password,
        returnSecureToken: true 
